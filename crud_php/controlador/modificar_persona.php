...
if (!empty($_POST['btnregistrar'])) { //si el boton registrar no esta vacio
    ...
    and !empty($_POST['correo'])) {;
        ...
        $sql =$conexion->query("UPDATE persona SET nombre='$nombre', apellido='$apellido', dni='$dni', fecha_nac='$fecha_nacimiento', correo='$correo' WHERE id_persona = $id"); //modificar datos de la tabla personas
        if ($sql==1) {
